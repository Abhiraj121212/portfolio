...
const About = ({ isDarkMode }) => {
  return (
    <motion.div id='about' className='w-full px-[7%] py-10 scroll-mt-20'
          ...
          <ul className='grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-2xl'>
            {infoList.map(({ icon, iconDark, title, description }, index) => (
              <motion.li 
                ...
                /* Changed cursor-pointer to cursor-default to prevent hand icon */
                className='border-[0.5px] border-gray-400 rounded-xl p-6 cursor-default hover:bg-lightHover hover:-translate-y-1 duration-500 hover:shadow-black dark:border-white/45 dark:hover:shadow-white dark:hover:bg-darkHover/50' 
                key={index}>
          ...
          <ul className='flex items-center gap-3 sm:gap-5'>
            {toolsData.map((tool, index) => (
              <motion.li 
                ...
                /* Added cursor-default here as well */
